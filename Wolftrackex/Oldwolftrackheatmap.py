...
def get_angle(kt,angle_in_degrees):
    angle = 2*np.pi*np.random.rand(len(kt))
    M = np.cosh(kt)/(2*np.pi*i0(kt))
    u = np.random.rand(len(kt))
    placeholder =  q(angle,kt,angle_in_degrees)/M
    
    indices = np.where((u > placeholder))
    while (u > placeholder).any():
        #for n in indices[0]:
        #   angle[n]= 2*np.pi*np.random.rand()
        angle[indices[0]] = 2 * np.pi * np.random.rand(len(indices[0]))
        placeholder[indices[0]] =  q(angle[indices[0]],kt[indices[0]],angle_in_degrees[indices[0]])/M[indices[0]]         
        indices = np.where(u > placeholder)       
...
#new way
def findxy2(var0,var1,var2,var3):
...
if __name__ == "__main__":
    ...
    rounds = 2#how many eps 

    th_start = 2*np.pi*np.random.rand(rounds) # starting point
    r_start = np.sqrt(np.random.rand(rounds))#  starting point
    ...
    while any(escape_time==0):
        ...
        kfun,angle_in_degrees = k(xnew[n],ynew[n])
